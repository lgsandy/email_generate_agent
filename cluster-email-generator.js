/**
 * Cluster Email Generator - Creates 5 distinct pharma marketing HTML emails from clusterData.json.
 *
 * Each version has a unique modern design theme with a fixed color palette.
 * Content placement: key_claims → supporting_claims → reusable_texts → components → references → footnotes.
 *
 * Usage: bun run cluster-email-generator.js [clusterData.json]
 */

import { generateText, Output } from 'ai';
import { z } from 'zod';
import { model } from './azure.js';
import { CLUSTER_EMAIL_SYSTEM_PROMPT, VERSION_THEMES, buildClusterEmailPrompt } from './prompts/cluster-email-prompt.js';
...
/**
 * Generate a single email version from cluster data.
...
export async function generateEmailVersion(clusterData, versionNumber) {
  ...
  const prompt = buildClusterEmailPrompt(clusterData, versionNumber);
  ...
  const { output } = await generateText({
    ...
    system: CLUSTER_EMAIL_SYSTEM_PROMPT,
    prompt,
    temperature: 0.3,
  });
...
async function main() {
  ...
  for (let i = 0; i < htmlResults.length; i++) {
    const outputPath = `./output/cluster-email-version-${i + 1}.html`;
